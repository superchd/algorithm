...
def my_sol(p, q, r, s, depth, flag):
    ...
    if flag == "L1":
    #    오른쪽으로 가야한다
        if direction(p, q, r, s, flag)    == "Right" : my_sol(mid(p, q), q, r, s, depth + 1, "L1")
    #    왼쪽으로 가야한다
        elif direction(p, q, r, s, flag)  == "Left"  : my_sol(p, mid(p, q), r, s, depth + 1, "L1")
        elif direction(p, q, r, s, flag) == "change" : my_sol(p, q, r, s, depth, "L2")
    # flag = -1 이면, 직선 L2에 있는 점q를 오른쪽으로 옮길지 왼쪽으로 옮길지 정함 
    else :
    #    오른쪽으로 가야한다
        if direction(p, q, r, s, flag) == "Right"    : my_sol(mid(p, q), q, r, s, depth + 1, "L2")
    #    왼쪽으로 가야한다
        elif direction(p, q, r, s, flag) == "Left"   : my_sol(p, mid(p, q), r, s, depth + 1, "L2")
        elif direction(p, q, r, s, flag) == "change" : my_sol(p, q, r, s, depth, "L2")
    # flag = -1 이면, 직선 L2에 있는 점q를 오른쪽으로 옮길지 왼쪽으로 옮길지 정함 
